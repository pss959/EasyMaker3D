...
// This struct is used to specify details of the construction of a polyline
// shape for BuildPolyLineShape().
struct PolyLineSpec : public ShapeSpec {
  std::vector<math::Point3f> vertices;    // Locations of vertices.
};

// Builds and returns a Shape representing a polyline.
ION_API const gfx::ShapePtr BuildPolyLineShape(const PolyLineSpec& spec);
