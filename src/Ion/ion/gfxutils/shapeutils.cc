...
static gfx::BufferObjectPtr BuildPolyLineBufferObject(
  ...
  for (size_t i = 0; i < vertex_count; ++i) {
    verts[i].position = spec.vertices[i];
    if (HasNormals(spec))
      verts[i].normal.Set(0, 0, 1);  // Hard-wired normal.
    if (HasTextureCoordinates(spec))
      verts[i].texture_coords.Set(static_cast<float>(i) * tc_scale, 0.f);
  }
  return BuildBufferObject(spec, vertex_count, verts);
}
...
const gfx::ShapePtr BuildPolyLineShape(const PolyLineSpec& spec) {
  gfx::ShapePtr shape(new(spec.allocator) gfx::Shape);
  shape->SetLabel("PolyLine");
  shape->SetPrimitiveType(gfx::Shape::kLineStrip);
  shape->SetAttributeArray(
      BuildAttributeArray(spec, BuildPolyLineBufferObject(spec)));
  // No need for an IndexBuffer.
  return shape;
}
