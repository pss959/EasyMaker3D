...
void OpenXRVR::InitInput_() {
    ...
    CHECK_XR_(xrStringToPath(instance_, "/user/hand/left",
                             &hand_state[HandIndex(Hand::kLeft)]->path));
    CHECK_XR_(xrStringToPath(instance_, "/user/hand/right",
                             &hand_state[HandIndex(Hand::kRight)]->path));
...
void OpenXRVR::CreateInputAction_(const char *name, XrAction &action) {
...
void OpenXRVR::AddControllerBindings() {
    std::vector<InputBinding_> bindings;

    for (int i = 0; i < 2; ++i) {
        std::string base_path =
            std::string("/user/hand/") + HandName(i) + "/input/";
...
OpenXRVR::InputBinding_ OpenXRVR::BuildInputBinding_(
...
void OpenXRVR::PollInput_(std::vector<Event> &events) {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        if (pinch_state.changedSinceLastSync) {
            ...
            event.flags.Set(pinch_state.isActive ? Event::Flag::kButtonPress :
        ...
        if (menu_state.changedSinceLastSync) {
            ...
            event.device = i == 0 ? Event::Device::kLeftController :
                Event::Device::kRightController;
            event.flags.Set(menu_state.isActive ? Event::Flag::kButtonPress :
                            Event::Flag::kButtonRelease);
        ...
        if (grip_state.changedSinceLastSync) {
            ...
            event.device = i == 0 ? Event::Device::kLeftController :
                Event::Device::kRightController;
            event.flags.Set(grip_state.isActive ? Event::Flag::kButtonPress :
                            Event::Flag::kButtonRelease);
