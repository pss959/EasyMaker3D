...
/// \ingroup Tests
class ConvertedModelTest : public SceneTestBase {};

// ----------------------------------------------------------------------------
// Because ConvertedModel is abstract, these tests use the derived concrete
// BeveledModel and TaperedModel classes. BeveledModel is derived from
// ScaledConvertedModel and TaperedModel is derived directly from
// ConvertedModel (and does not have specialized transform syncing).
// ----------------------------------------------------------------------------

TEST_F(ConvertedModelTest, ChangeOperandModel) {
...
TEST_F(ConvertedModelTest, SyncTransforms) {
...
TEST_F(ConvertedModelTest, SyncTransformsScaled) {
    auto box = Model::CreateModel<BoxModel>();
    box->SetUniformScale(4);
    box->SetTranslation(Vector3f(0, 4, 0));
...
TEST_F(ConvertedModelTest, Copy) {
    auto box = Model::CreateModel<BoxModel>();
    box->SetUniformScale(4);
    box->SetTranslation(Vector3f(0, 4, 0));
...
TEST_F(ConvertedModelTest, CopyScaled) {
    auto box = Model::CreateModel<BoxModel>();
    box->SetUniformScale(4);
    box->SetTranslation(Vector3f(0, 4, 0));
...
TEST_F(ConvertedModelTest, Errors) {
    // These test that the ConvertedModel parent/child functions assert when
    // called directly.
    auto box      = Model::CreateModel<BoxModel>();
    auto tapered = Model::CreateModel<TaperedModel>();
