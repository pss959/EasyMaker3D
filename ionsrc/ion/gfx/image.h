...
class ION_API Image : public base::Notifier {
 public:
  ...
  enum Format {
    ...
    /// RGBA 32-bit floating point image (for OpenGL ES 2.0 compatibility,
    /// where both format and internal_format are GL_RGBA).
    kRgbaFloat,
    ...
    /// Depth stencil rb, 24-bit depth, 8-bit stencil.
    kRenderbufferDepth24Stencil8,
    /// Depth stencil rb, 32-bit float depth, 8-bit stencil.
    ...
    /// Depth stencil image, 24-bit depth, 8-bit stencil.
    kTextureDepth24Stencil8,
    kTextureDepth32f,          ///< Depth image, 32-bit depth, float data.
    /// Depth stencil rb, 32-bit float depth, 8-bit stencil.
    kTextureDepth32fStencil8,
    ...
    kPvrtc1Rgba2,  ///< PVRTC1-compressed image (2 bits per pixel, with alpha).
    kPvrtc1Rgba4,  ///< PVRTC1-compressed image (4 bits per pixel, with alpha).
    ...
    // Packed sized images.
    //// RGB color image, 10 bits red and green, 11 / bits blue, packed
    /// 10f,11f,11f uint32 data.
    kRgb11f_11f_10f_Rev,
    /// RGB color image, 10 bits red and green, 11 bits blue, packed float
    /// data.
    kRgb11f_11f_10f_RevFloat,
    /// RGB color image, 10 bits red and green, 11 bits blue, packed half float
    /// data.
    ...
    /// uint16 data.
    kRgb5a1Short,
    /// RGBA color+alpha image, 5 bits per color, 1 bit alpha, packed
    /// 2,10,10,10 uint32 data.
    kRgb5a1Int,
    /// RGB color+alpha image, 9 bits per color, 5 bits exponent, float data.
    kRgb9e5Float,
    /// RGB color+alpha image, 9 bits per color, 5 bits exponent, half float
    /// data.
    kRgb9e5Half,
    /// RGB color+alpha image, 9 bits per color, 5 bits exponent, packed 5999
    /// uint32 data.
    kRgb9e5RevInt,
    /// RGBA color+alpha image, 4 bits each, uint8 data.
    kRgba4Byte,
    /// RGBA color+alpha image, 4 bits each, packed 4444 uint16 data.
    kRgba4Short,

    /// A texture backed by an EGLImage, which may have arbitrary format.
    kEglImage,
